...
import { MyCalendarComponent } from './components/calendar/calendar.component';
...
export class DettaglioDipendenteComponent implements OnInit {
  ...
  private reportNotificationService = inject(ReportNotificationService);
  ...
  notifica = signal<string | null>(null);



  ngOnInit() {
    this.reportNotificationService.reportPronto$.subscribe(data => {
      this.notifica.set(data.messaggio);

      if (data.tipo === 'annuale' && data.anno) {
        this.postItService.scaricaReportAnnuale(data.anno).subscribe({
          next: (blob) => saveAs(blob, `report_annuale_${data.anno}.xlsx`),
          error: () => alert(`Report ${data.anno} non ancora disponibile.`)
        });
      } else if (data.tipo === 'dipendente') {
        this.downloadexel();
      }
    });
    console.log(this.postItService.username); //Questo è il servizio inniettato 
    ...
    if (id) {
      this.dipendentiService.getDipendente(id).subscribe({
        next: (data) => {
          ...
          this.reportNotificationService.uniscitiAlGruppoUtente();
        },
        error: (err) => {
  ...
  generaReportAnnuale() {
    const anno = 2026;
    const connectionId = this.reportNotificationService.getConnectionId();
    if (!connectionId) {
      alert('Connessione SignalR non ancora pronta, riprova tra un secondo.');
      return;
    }
    this.postItService.generaReportAnnuale(anno, connectionId).subscribe({
      next: () => alert(`Report ${anno} in elaborazione, scaricalo tra qualche secondo.`),
      error: (err) => console.error('Errore nella generazione report:', err)
    });
  }

  downloadReportAnnuale() {
    const anno = 2026;
    this.postItService.scaricaReportAnnuale(anno).subscribe({
      next: (blob) => saveAs(blob, `report_annuale_${anno}.xlsx`),
      error: () => alert(`Report ${anno} non ancora disponibile, genera prima il report.`)
    });
  }
